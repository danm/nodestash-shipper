...
module.exports = class Aggregate {
    ...
    save(params, done) {
        ...
        save.on('close', () => {
            ...
            self.lines(params.Key, true, (nolines) => {
    ...
    read(key, nolines, done) {
        ...
        stream.on('line', (line) => {
            ...
            fs.appendFileSync('output.tmp', meta + '\n');
            fs.appendFileSync('output.tmp', JSON.stringify(obj) + '\n');
    ...
    distribute(done) {
        let self = this;
        self.c.a('Sending to Mongo', 1, self.key);
        self.mongo.ship(() => {
            self.c.a('Mongo Receieved', 1, self.key);
            self.elastic.ships3(self.key, (err) => {
                if (err) throw err;
                self.cleanup(done);
            });
        });
