...
const execute = (inserts, article, database, cps, cb) => {
    console.log(article);
    cps.update(inserts[article][0], inserts[article][1], inserts[article][2], (err) => {
        if (err) { throw err; }
        article++;
...
module.exports = class Elastic {
    ...
    getServers(cb) {
        ...
        if (self.environment === 'sandbox') {
            self.connection = 'mongodb://192.168.0.16:27017/' + self.databasename;
            cb(null);
        } else {
    ...
    ship(cb) {
                ...
                case 'bbc':
                    {
                        if (line.ref_name !== 'homepage') {
                            aggregate(doc, line.ref_page, 'bbc_' + line.ref_name);
                        }
        ...
        stream.on('close', () => {
            ...
            for (let id in articles) {
                let pusher = [];
                for (let time in articles[id]) {
                    pusher.push(articles[id][time]);
                }
                documents.push([{ _id: parseInt(id) }, { $push: { analytics: { $each: pusher } } }, { upsert: true }]);
            }

            self.app.c.a("Shipping Documents to Mongo");
            arrange(documents, 0, self.database, undefined, cb);
        });
    }
};