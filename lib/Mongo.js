...
module.exports = class Mongo {
    ...
    getServers(cb) {
        ...
        if (self.app.config.environment === 'sandbox') {
            ...
            let filter = { "Filters": [{ "Name": "tag:BBCComponent", "Values": ["mongo-db"] }, { "Name": "instance-state-name", "Values": ["running"] }, { "Name": "tag:BBCEnvironment", "Values": [self.app.config.environment] }] };
            ...
            ec2.describeInstances(filter, function(err, res) {
                if (err) {
                    ...
                    res.Reservations.forEach(function(x, idx) {
                        self.connection += x.Instances[0].PrivateIpAddress + '/' + mongo.database;
