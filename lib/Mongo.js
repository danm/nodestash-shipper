...
const execute = (inserts, article, database, cps, cb) => {
    cps.update({ _id: inserts[article].id, type: { $exists: true } }, { $set: { updated: new Date() }, $push: { analytics: inserts[article].pushed } }, (err) => {
        if (err) {
            console.log('Error inserting into Mongo', article, err);
        }
        article++;
