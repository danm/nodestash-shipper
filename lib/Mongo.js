...
const arrange = (inserts, site, article, database, cb, cps) => {
    ...
    if (cps === undefined) {
        cps = database.collection(inserts[site].name);
    }
    ...
    if (inserts[site].data[article] === undefined) {
        ...
        if (inserts[site + 1] === undefined) {
            ...
            cps = database.collection(inserts[site + 1].name);
            ...
            for (let id in file[app]) {
                ...
                for (let time in file[app][id]) {
                    for (let obj in file[app][id][time]) {
                        if (obj === 'views') {
                            incs['dax.' + time + '.' + obj] = file[app][id][time][obj];
                        } else if (obj === 'date') {
                            sets['dax.' + time + '.date'] = new Date(file[app][id][time][obj]);
                        } else {
                            for (let param in file[app][id][time][obj]) {
                                incs['dax.' + time + '.' + obj + '.' + param] = file[app][id][time][obj][param];
                            }
                        }
                    }
                }

                inserts[loop].data.push([{ _id: id }, { $inc: incs, $set: sets }, { upsert: true }]);
            }
            loop++;
        }
