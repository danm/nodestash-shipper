...
const arrange = (inserts, article, database, cps, bulk, cb) => {

    if (cps === undefined) {
        cps = database.collection('cps');
        bulk = cps.initializeOrderedBulkOp();
    }

    if (article % 10000 === 0 && article !== 0) {
        console.log('Sending ' + article + '/' + inserts.length);
        bulk.execute(function(err, result) {
            console.log(err, result);
            bulk = cps.initializeOrderedBulkOp();
            execute(inserts, article, database, cps, bulk, cb);
        });
        ...
        console.log('Sending last' + article + '/' + inserts.length);
        bulk.execute(function(err, result) {
            console.log(err, result);
            cb();
        });
...
const execute = (inserts, article, database, cps, bulk, cb) => {

    console.log("started " + new Date(), inserts[article].id, inserts[article].date);

    bulk.find({ _id: inserts[article].id, "analytics.date": inserts[article].date }).upsert().update({ $inc: inserts[article].inc });
    article++;
    arrange(inserts, article, database, cps, bulk, cb);

    // cps.update({ _id: inserts[article].id, "analytics.date": inserts[article].date }, { $inc: inserts[article].inc }, { upsert: true }, (err) => {

    //     if (err) {
    //         //likely it doesn't exist so we need to create it
    //         console.log("continueing " + new Date(), inserts[article].id, inserts[article].date);
    //         cps.update({ _id: inserts[article].id }, { $push: { analytics: inserts[article].pushed } }, { upsert: true }, (err) => {
    //             if (err) throw err;
    //             // console.log(`${article} / ${inserts.length} pushed`);
    //             console.log("contend " + new Date(), inserts[article].id, inserts[article].date);
    //             article++;
    //             arrange(inserts, article, database, cps, bulk, cb);
    //         });
    //     } else {
    //         console.log("end " + new Date(), inserts[article].id, inserts[article].date);
...
module.exports = class Mongo {
    ...
    getServers(cb) {
        ...
        if (self.app.config.environment === 'sandbox') {
    ...
    ship(cb) {
        ...
        stream.on('close', () => {
            ...
            arrange(pusher, 0, self.database, undefined, undefined, cb);
        });
    }
};